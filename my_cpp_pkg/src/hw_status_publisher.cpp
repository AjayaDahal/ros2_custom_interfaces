...
class HardwareStatusPublisher: public rclcpp::Node{
    public:
        HardwareStatusPublisher():Node("hardware_status_publisher"){
            ...
            timer_ = this->create_wall_timer(std::chrono::seconds(1), 
                                             std::bind(&HardwareStatusPublisher::publisherHardwareStatus, this));
    ...
    private:
        void publisherHardwareStatus(){
            ...
            msg.debug_message = "Motors not ready due to high temp.";
            pub_->publish(msg);
        }
...
int main(int argc, char **argv){
    ...
    auto node = 
    std::make_shared<HardwareStatusPublisher>();
